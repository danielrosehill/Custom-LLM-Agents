...
def update_readme(readme_path, new_files):
    if not os.path.exists(readme_path):
        content = "# Repository Documentation\n\n## Index\n\n| Date | File Name | Path | Hugging Face Badge |\n|------|-----------|------|------------------|\n"
    else:
    ...
    if not index_match:
        ...
        table_header_exists = bool(re.search(r'\|\s*Date\s*\|\s*File Name\s*\|\s*Path\s*\|\s*Hugging Face Badge\s*\|', content[index_pos:]))
    ...
    if not table_header_exists:
        new_content = "| Date | File Name | Path | Hugging Face Badge |\n|------|-----------|------|------------------|\n"

    for creation_time, file_name, rel_path, badge in new_files:
        if rel_path not in existing_entries:
            new_content += f"| {creation_time.strftime('%Y-%m-%d %H:%M:%S')} | {file_name} | [link]({rel_path}) | {badge} |\n"
            existing_entries.add(rel_path)

    if new_content:
        if table_header_exists:
            table_header_match = re.search(r'\|\s*Date\s*\|\s*File Name\s*\|\s*Path\s*\|\s*Hugging Face Badge\s*\|\s*\n\|[-\s|]*\n', content[index_pos:])
